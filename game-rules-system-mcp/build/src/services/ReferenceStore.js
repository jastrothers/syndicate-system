...
export async function saveReference(name, game, version, type, tags, content, deleted = false) {
    ...
    const metadata = { title: name, type, tags, lastUpdated: now };
    if (game !== undefined)
        metadata.game = resolvedGame;
...
export async function getReference(name, game, version) {
    ...
    if (game) {
        ...
        row = stmt.get(name, name, resolvedVersion);
