...
export const drawFromDeckTool = {
    ...
    handler: async (args) => {
        ...
        if (!Array.isArray(session.state[args.targetHandId]))
            session.state[args.targetHandId] = [];
...
export const moveEntityTool = {
    ...
    handler: async (args) => {
        const session = await getSession(args.sessionId);
        if (!Array.isArray(session.state[args.sourceId]))
            session.state[args.sourceId] = [];
        if (!Array.isArray(session.state[args.targetId]))
            session.state[args.targetId] = [];
        ...
        const idx = srcArray.findIndex((item) => item === args.entityId || (item && (item.id === args.entityId || item.name === args.entityId)));
        if (idx === -1) {
