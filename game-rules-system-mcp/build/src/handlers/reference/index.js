...
export const saveReferenceTool = {
    name: "save_reference",
    ...
    handler: async (args) => {
        if (args.batch && args.batch.length > 0) {
            ...
            for (const ref of args.batch) {
                await ReferenceStore.saveReference(ref.name, ref.game || args.game, ref.version || args.version, ref.type, ref.tags, ref.content);
                names.push(ref.name);
            }
...
export const getReferenceTool = {
    ...
    handler: async (args) => {
        ...
        if (!ref) {
            ...
            throw new Error(`Reference '${args.name}' not found${hint}${versionHint}. Try list_references to see available references, or check game/version filters.`);
        }
        return jsonResponse(ref);
    },
};
export const listReferencesTool = {
...
export const rebuildReferenceIndexTool = {
...
export const deleteReferenceTool = {
