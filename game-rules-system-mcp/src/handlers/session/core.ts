  ...
  handler: async (args) => {
    ...
    if (args.keysOnly) {
      const summary = Object.fromEntries(
        Object.keys(state).map(k => [k, Array.isArray(state[k])
          ? `[${(state[k] as unknown[]).length} items]`
          : typeof state[k]])
      );
    ...
    if (args.fields) {
      const filtered = Object.fromEntries(
        args.fields.filter((f: string) => f in state).map((f: string) => [f, state[f]])
      );
...
export const updateGameStateTool: ToolDefinition = {
  ...
  handler: async (args) => {
    ...
    const stateSnapshot = Object.fromEntries(
      Object.keys(session.state).map(k => [k, Array.isArray(session.state[k])
        ? `[${(session.state[k] as unknown[]).length} items]`
