...
async function instantiateDeckFromReference(session: any, deckId: string, referenceName: string, game?: string, version?: string, shuffle: boolean = true) {
  ...
  if (!ref) throw new Error(`Reference '${referenceName}' not found.`);
...
export const evaluateGameStateTool: ToolDefinition = {
...
export const setupGameFromManifestTool: ToolDefinition = {
...
export const executeMacroActionTool: ToolDefinition = {
  ...
  handler: async (args) => {
    ...
    const context = vm.createContext({
      ...
      inputs: args.inputs || {},
      api: {
        draw: (deckId: string, handId: string, count: number) => {
           if (!Array.isArray(session.state[deckId])) session.state[deckId] = [];
           if (!Array.isArray(session.state[handId])) session.state[handId] = [];
           const deck = session.state[deckId] as any[];
           const hand = session.state[handId] as any[];
           const drawn = deck.splice(0, count);
           hand.push(...drawn);
           modified = true;
           return drawn;
        },
        move: (entityId: string, sourceId: string, targetId: string) => {
           if (!Array.isArray(session.state[sourceId])) return false;
           if (!Array.isArray(session.state[targetId])) session.state[targetId] = [];
           const source = session.state[sourceId] as any[];
           const target = session.state[targetId] as any[];
        ...
        shuffle: (deckId: string) => {
           if (!Array.isArray(session.state[deckId])) return;
           shuffleArray(session.state[deckId] as any[]);
           modified = true;
        },
        log: (msg: string) => {
