...
test("Cleanup Integration Tests", async (t) => {
    ...
    await t.test("delete_reference (soft): tombstones reference so it no longer appears in queries", async () => {
        ...
        const refsData = JSON.parse(listResult.content[0].text);
        const found = refsData.items.find((r) => r.name === "soft-delete-ref");
    ...
    await t.test("delete_reference (hard): permanently removes file and index entry", async () => {
        ...
        const refsData = JSON.parse(listResult.content[0].text);
        const found = refsData.items.find((r) => r.name === "hard-delete-ref");
    ...
    await t.test("delete_rulebook_version: removes a versioned snapshot from the rulebook", async () => {
        ...
        const beforeData = JSON.parse(listBefore.content[0].text);
        assert.ok(beforeData.versions.some((v) => v.versionTag === "0.1.0"), "Version should exist before deletion");
        const deleteResult = await client.callTool({
        ...
        const afterData = JSON.parse(listAfter.content[0].text);
        assert.ok(!afterData.versions.some((v) => v.versionTag === "0.1.0"), "Version should be gone after deletion");
    });
    await t.test("delete_rulebook_version: rejects 'latest' as a version tag", async () => {
        ...
        assert.strictEqual(result.isError, true);
        assert.ok(result.content[0].text.includes("latest"), "Error should mention 'latest'");
    });
    ...
    await t.test("delete_game: aborts when confirm is false", async () => {
        ...
        assert.strictEqual(result.isError, true);
        assert.ok(result.content[0].text.includes("confirm"), "Error should mention confirm");
    });
    ...
    await t.test("delete_game: wipes game directory, sessions, and references", async () => {
        ...
        const sessionId = JSON.parse(sessionResult.content[0].text).sessionId;
        await client.callTool({
            name: "save_reference",
        ...
        const sessionCheck = await client.callTool({
            name: "get_game_state",
            arguments: { sessionId },
        });
        ...
        const refList = await client.callTool({
            name: "list_references",
            arguments: { game: gameName },
        });
        const refsData = JSON.parse(refList.content[0].text);
        ...
        const rbList = await client.callTool({
            name: "list_rulebooks",
            arguments: {},
        });
        const rbData = JSON.parse(rbList.content[0].text);
