...
test("Card Mechanics Integration Tests", async (t) => {
    ...
    await t.test("peek_at_deck (top)", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        const peeked = JSON.parse(result.content[0].text);
        ...
        const state = JSON.parse(stateRes.content[0].text);
    ...
    await t.test("search_zone", async () => {
        ...
        const names = data.results.map((r) => r.item.name);
    ...
    await t.test("insert_into_deck", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        const peekRes = await client.callTool({
            name: "peek_at_deck",
            arguments: { sessionId: cardSessionId, deckId: "deck", count: 1 },
        });
        const top = JSON.parse(peekRes.content[0].text);
    ...
    await t.test("buy_card macro (success)", async () => {
        ...
        const afterState = JSON.parse(afterRes.content[0].text);
    ...
    await t.test("buy_card macro (insufficient resources)", async () => {
        ...
        assert.ok(result.content[0].text.includes("Insufficient resources"));
    });
    ...
    await t.test("reveal_cards (specific)", async () => {
        ...
        const ledgerRes = await client.callTool({
            name: "get_action_history",
            arguments: { sessionId: cardSessionId },
        });
        const ledgerData = JSON.parse(ledgerRes.content[0].text);
        const ledger = ledgerData.items ?? ledgerData;
        const revealActions = ledger.filter((a) => a.actionType === "reveal_cards");
