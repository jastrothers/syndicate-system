...
test("E2E Simulated Agent: Rule Ingestion", async (t) => {
    ...
    await t.test("Agent parsing and saving Movement rules", async () => {
        ...
        assert.ok(result.content[0].text.includes("Successfully saved"), "Agent should be able to save movement rule");
    });
    ...
    await t.test("Agent isolating specific spells as templates", async () => {
        const spell1 = await client.callTool({
            name: "save_reference",
            arguments: {
                name: "spell_fireball",
                type: "spell",
        ...
        assert.ok(spell1.content[0].text.includes("Successfully saved"));
        const spell2 = await client.callTool({
            name: "save_reference",
            arguments: {
                name: "spell_heal",
                type: "spell",
        ...
        assert.ok(spell2.content[0].text.includes("Successfully saved"));
    });
    ...
    await t.test("Suite validates the ingested rules via complex reference DB querying", async () => {
        ...
        const spellQuery = await client.callTool({
            name: "list_references",
            arguments: { type: "spell" }
        });
        const spellsData = JSON.parse(spellQuery.content[0].text);
        assert.strictEqual(spellsData.count, 2, "Agent should have saved exactly 2 spells");
        assert.ok(spellsData.items.some((s) => s.name === "spell_fireball"));
        // Find all references tagged 'combat'
        const combatQuery = await client.callTool({
            name: "list_references",
            arguments: { tags: ["combat"] }
        });
        const combatData = JSON.parse(combatQuery.content[0].text);
        ...
        const getMovement = await client.callTool({
            name: "get_reference",
            arguments: { name: "movement_core" }
        });
        const movementModule = JSON.parse(getMovement.content[0].text);
