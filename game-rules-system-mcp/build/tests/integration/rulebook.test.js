...
test("General & Rulebook Integration Tests", async (t) => {
    ...
    await t.test("Compile markdown rulebook", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.ok(result.content[0].text.includes("Successfully compiled Markdown rulebook"));
    });
    await t.test("Delete rule", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.ok(result.content[0].text.includes("Successfully deleted rule section"));
    });
    ...
    await t.test("Full Rulebook Display", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.ok(result.content[0].text.includes("# Untitled Board Game"));
    });
});
