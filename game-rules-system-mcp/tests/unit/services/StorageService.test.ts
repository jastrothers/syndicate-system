...
describe("StorageService", () => {
  ...
  it("should throw error when reading non-existent file without default", async () => {
    await assert.rejects(
      StorageService.readJson(path.join(TEST_DIR, "missing.json")),
      /ENOENT/
    );
  });
  ...
  it("should return default value when reading non-existent file with default", async () => {
    ...
    const read = await StorageService.readJson(
      path.join(TEST_DIR, "missing.json"),
      defaultValue
    );
  ...
  it("should throw when reading malformed JSON without default", async () => {
    ...
    await assert.rejects(
      StorageService.readJson(badFile),
      (err: Error) => err instanceof SyntaxError || err.message.includes("JSON")
    );
  });
  ...
  it("should overwrite existing file content", async () => {
    ...
    const read = await StorageService.readJson<{ version: number }>(TEST_FILE);
