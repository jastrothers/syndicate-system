...
test("Playtesting Session Integration Tests", async (t) => {
    ...
    await t.test("Create Session", async () => {
        ...
        const sessionStr = result.content[0].text;
    ...
    await t.test("Update and Get Game State", async () => {
        ...
        const state = JSON.parse(getResult.content[0].text);
    ...
    await t.test("Draw from deck, Shuffle, Move", async () => {
        ...
        const stateBeforeMove = await client.callTool({ name: "get_game_state", arguments: { sessionId: currentSessionId } });
        const stateData = JSON.parse(stateBeforeMove.content[0].text);
        ...
        const finalStateReq = await client.callTool({ name: "get_game_state", arguments: { sessionId: currentSessionId } });
        const finalState = JSON.parse(finalStateReq.content[0].text);
    ...
    await t.test("Roll Dice macro & Get Ledger", async () => {
        ...
        const diceResult = await client.callTool({
        ...
        const data = JSON.parse(diceResult.content[0].text);
    ...
    await t.test("Log playtest note", async () => {
        ...
        assert.strictEqual(result.isError, undefined);
        assert.ok(result.content[0].text.includes("Successfully appended note"));
    });
});
